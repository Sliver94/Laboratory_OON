...
#sistemare latency
    def find_best_latency(self, node_input, node_output):
        ...
        for i in range(len(row_index)):
            current_latency = self.weighted_paths['latency'][row_index[i]]
            if i == 0:
...
#sistemare per non far uscire in e out uguali
    for i in range(100):
    ...
    for i in range(len(input_node)):
        best_snr_path_index_global = network.find_best_snr(input_node[i], output_node[i])
        print(best_snr_path_index_global)
        print('For input = ', input_node[i], ', output = ', output_node[i], ', the path with the best SNR is: ',
              df['path'][best_snr_path_index_global], ' and SNR =', df['snr'][best_snr_path_index_global])
        connection_list.append(Connection(input_node[i], output_node[i], 0.001))

    network.stream(connection_list, snr_or_latency)
